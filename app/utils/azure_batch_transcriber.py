...
class TranscriptionJob:
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> "TranscriptionJob":
        ...
        return cls(
            ...
            audio_url=data.get("contentUrls", [""])[0]
            if data.get("contentUrls")
            else "",
            self_url=data["self"],
...
class AzureBatchTranscriber:
    ...
    def __init__(
        self, speech_key: Optional[str] = None, speech_region: Optional[str] = None
    ):
        """
    ...
    async def upload_audio(self, file_path: str) -> tuple[str, str]:
        ...
        # Create blob client with upload settings tuned for resilience.
        #
        # max_single_put_size=8 MiB:
        #   The SDK uses a single PUT for files below this threshold; anything
        #   larger is split into blocks (Put Block + Put Block List).  The
        #   typical audio files we upload are 20-40 MiB, so lowering this from
        #   the default 64 MiB ensures they are always uploaded in blocks.
        #   This is critical because a write-timeout on a single-PUT request
        #   requires retrying the entire file; with block upload only the
        #   stalled block needs to be retried.
        #
        # max_block_size=4 MiB:
        #   Each block is 4 MiB.  A 36 MiB file becomes ~9 blocks.
        #
        # max_concurrency=2:
        #   Number of block-upload threads in parallel per file.  With
        #   max_single_put_size=8 MiB this value now actually takes effect
        #   (previously all files were below 64 MiB and used single PUT, so
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    # Use longer delays with ±20 % jitter.  Write-timeout errors
                    # are typically caused by transient Azure storage throttling
                    # or a frozen TCP window; a short 2 s delay is not enough
                    # for the backend to recover.
                    base_wait = 10 * (2 ** (attempt - 1))  # 10s, 20s
        ...
        logger.info(
            f"Uploaded {file_size_mb:.1f} MB to blob in {upload_duration:.1f}s: {blob_name}"
        )
        ...
        account_key = (
            ...
            if blob_service_client.credential
            else None
        )

        if not account_name or not account_key:
            raise ValueError(
                "Could not retrieve storage account credentials for SAS generation"
            )
        ...
        logger.info("Generated SAS URL for blob")
    ...
    async def create_transcription(
        ...
        if display_name is None:
            display_name = (
                f"SubGen-Azure-Batch-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )
        ...
        if candidate_locales:
            ...
            payload["languageIdentification"] = {
                "candidateLocales": candidate_locales[
                    :4
                ],  # Enforce max 4 for Single mode
                "mode": "Single",
            }
            logger.debug(
                f"Language identification enabled with candidates: {candidate_locales[:4]}"
            )
        ...
        logger.debug(
            ...
            if len(audio_url) > 100
            else f"Audio URL: {audio_url}"
        )
        ...
        async with session.post(url, headers=self.headers, json=payload) as response:
            if response.status != 201:
                ...
                raise RuntimeError(
                    f"Failed to create transcription: {response.status} - {error_text}"
                )
    ...
    async def get_transcription_result(self, job_id: str) -> TranscriptionResult:
        ...
        async with session.get(content_url) as response:
            if response.status != 200:
                error_text = await response.text()
                raise RuntimeError(
                    f"Failed to download transcription result: {response.status} - {error_text}"
                )
    ...
    async def wait_for_transcription(
        ...
        while True:
            ...
            if job.status == TranscriptionStatus.FAILED:
                raise RuntimeError(
                    f"Transcription job {job_id} failed: {job.error_message}"
                )
    ...
    async def delete_transcription(self, job_id: str) -> None:
        ...
        async with session.delete(url, headers=self.headers) as response:
            if response.status not in (200, 204):
                ...
                if (
                    "DeleteNotAllowed" in error_text
                    or "hasn't finished yet" in error_text
                ):
                    logger.info(
                        f"Transcription {job_id} still running on Azure (will complete/expire automatically)"
                    )
                else:
                    logger.warning(
                        f"Failed to delete transcription {job_id}: {response.status} - {error_text}"
                    )
            else:
    ...
    async def list_transcriptions(self, top: int = 100) -> List[TranscriptionJob]:
        ...
        async with session.get(url, headers=self.headers) as response:
            if response.status != 200:
                error_text = await response.text()
                raise RuntimeError(
                    f"Failed to list transcriptions: {response.status} - {error_text}"
                )

            data = await response.json()
            return [
                TranscriptionJob.from_api_response(item)
    ...
    async def get_supported_locales(self) -> List[str]:
        ...
        async with session.get(url, headers=self.headers) as response:
            if response.status != 200:
                error_text = await response.text()
                raise RuntimeError(
                    f"Failed to get supported locales: {response.status} - {error_text}"
                )
...
async def transcribe_audio(
    audio_path: str, language: str = "en-US", output_srt_path: Optional[str] = None
) -> str:
    """
