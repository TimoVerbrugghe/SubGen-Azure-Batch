...
class TranscriptionConfig:
    ...
    def preferred_audio_languages_list(self) -> list:
        ...
        return [
            lang.strip().lower()
            for lang in self.preferred_audio_languages.split("|")
            if lang.strip()
        ]
...
class SkipConfig:
    ...
    def audio_language_skip_list(self) -> List[str]:
        ...
        return [
            lang.strip().lower()
            for lang in self.skip_if_audio_track_is.split("|")
            if lang.strip()
        ]
    ...
    def subtitle_languages_skip_list(self) -> List[str]:
        ...
        return [
            lang.strip().lower()
            for lang in self.skip_subtitle_languages.split("|")
            if lang.strip()
        ]
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            # Skip configuration
            skip=SkipConfig(
                skip_if_target_subtitles_exist=get_bool(
                    os.getenv("SKIP_IF_TARGET_SUBTITLES_EXIST", "true")
                ),
                skip_if_external_subtitles_exist=get_bool(
                    os.getenv("SKIP_IF_EXTERNAL_SUBTITLES_EXIST", "false")
                ),
                skip_if_internal_subtitles_language=os.getenv(
                    "SKIP_IF_INTERNAL_SUBTITLES_LANGUAGE", ""
                ),
                skip_only_subgen_subtitles=get_bool(
                    os.getenv("SKIP_ONLY_SUBGEN_SUBTITLES", "false")
                ),
                skip_if_audio_track_is=os.getenv("SKIP_IF_AUDIO_TRACK_IS", ""),
                skip_subtitle_languages=os.getenv("SKIP_SUBTITLE_LANGUAGES", ""),
                skip_unknown_language=get_bool(
                    os.getenv("SKIP_UNKNOWN_LANGUAGE", "false")
                ),
                skip_if_no_language_but_subtitles_exist=get_bool(
                    os.getenv("SKIP_IF_NO_LANGUAGE_BUT_SUBTITLES_EXIST", "false")
                ),
            ),
            # Subtitle naming configuration
            subtitle_naming=SubtitleNamingConfig(
                naming_type=os.getenv(
                    "SUBTITLE_LANGUAGE_NAMING_TYPE", "ISO_639_2_B"
                ).upper(),
                show_subgen_marker=get_bool(
                    os.getenv("SHOW_IN_SUBNAME_SUBGEN", "false")
                ),
            ...
            transcription=TranscriptionConfig(
                ...
                limit_to_preferred_audio_languages=get_bool(
                ...
                language_detection_candidates=os.getenv(
